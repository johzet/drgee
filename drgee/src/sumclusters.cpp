...
// [[Rcpp::export]]
RcppExport SEXP sum_clust(SEXP u, SEXP ids, SEXP uids, SEXP nclusts){
  NumericMatrix u_tmp(u);
  IntegerVector id_tmp(ids);
  IntegerVector uid_tmp(uids);
  int n = id_tmp.length();
  int m = uid_tmp.length();
